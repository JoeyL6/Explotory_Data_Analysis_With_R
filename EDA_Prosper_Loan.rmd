...
ggplot(loan_df,aes(x=reorder(BorrowerState,BorrowerState,
                     function(x)+length(x)))) + 
        geom_bar() + coord_flip() + ggtitle("Counts of loans by US states") +
  xlab("States' name abbreviated") + ylab("Number of Loans at Prosper Marketplace")
...
ggplot(loan_df,aes(x=reorder(Occupation,Occupation,
                     function(x)+length(x)))) + 
        geom_bar() + coord_flip() + ggtitle("Total number of loans by occupation") +
  xlab("Occupation") + ylab("Number of Loans")
```
...
Although not all features in this analysis is numeric, its still pretty reasonalbe to start the analysis with pearson correlation matrix
...
## 4.2 Credit score, prosperscore, loan original amount and number of investors
...
ggplot(data = loan_df, aes(StatedMonthlyIncome, CreditScoreRangeUpper, colour = factor(DeliquencyState))) + 
geom_point(alpha = 0.1) + xlim(0, 10000) + labs(title ="Deliquency state vs Stated Monthly Income and Credit Score")
```
...
ggplot(data = loan_df, aes(x = DeliquencyState, y = LoanOriginalAmount)) + 
      geom_boxplot()+labs(title ="Deliquency state vs Stated Monthly Income and Credit Score")
```
